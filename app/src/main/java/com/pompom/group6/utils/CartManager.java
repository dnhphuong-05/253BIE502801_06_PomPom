...
public class CartManager {
    ...
    // Mỗi "chủ giỏ" (khách vãng lai hoặc một user_id cụ thể) có file riêng — tránh giỏ của
    // khách vãng lai bị hiện lại như giỏ của tài khoản vừa đăng nhập (và ngược lại).
    private static final String PREF_PREFIX = "pompom_cart_";
    private static final String OWNER_GUEST = "guest";
    ...
    public static synchronized CartManager getInstance(Context context) {
        if (instance == null) {
            ...
            instance.switchOwnerIfNeeded(context);
        }
        return instance;
    }

    /** Giỏ hàng gắn theo phiên đăng nhập hiện tại: đổi tài khoản (hoặc đăng xuất) → nạp lại đúng giỏ của chủ đó. */
    private void switchOwnerIfNeeded(Context context) {
        String userOid = Session.getUserOid(context);
        String ownerKey = userOid != null ? userOid : OWNER_GUEST;
        if (ownerKey.equals(currentOwnerKey)) return;
        currentOwnerKey = ownerKey;
        prefs = appContext.getSharedPreferences(PREF_PREFIX + ownerKey, Context.MODE_PRIVATE);
